...
def differential_evolution(img, func, bounds, args=(), strategy='best1bin',
    ...
    if args[0] == 0:
        ...
        solver = DifferentialEvolutionSolver(img, func, bounds, args=args, strategy=strategy, maxiter=maxiter,
                                             ...
                                             seed=seed, polish=polish, callback=callback, disp=disp, init=init,
                                             atol=atol)
        return solver.AdaptiveDynamic_solve_1()
...
class DifferentialEvolutionSolver(object):
    ...
    def dynamic_solve(self):
        ...
        for nit in range(1, self.maxiter + 1):
            ...
            self.itersize = max(0, min(self.num_population_members, self.maxfun - self._nfev + 1))
            #2
            for candidate in range(self.itersize):
                ...
                if self.args[1] != -1:
                    self.DEC(self.img, trial, self.args)
                parameter = np.array(self._scale_parameters(trial))
                energy = self.func(parameter, 1)
                self._nfev += 1
                if energy < self.population_energies[candidate]:
                    self.population[candidate] = trial
                    self.population_energies[candidate] = energy
                    # if the trial candidate also has a lower energy than the best solution then replace that as well
                    if energy < self.population_energies[0]:
                        ...
                        if warning_flag or intol:
                            break
            if candidate < self.itersize-1:
                break
        else:
            status_message = _status_message['maxiter']
            warning_flag = True

        DE_result = OptimizeResult(x=self.x, fun=self.population_energies[0], nfev=self._nfev,
                                   nit=nit+(candidate+1)/self.itersize,
                                   message=status_message, success=(warning_flag is not True))
        if self.polish:
    ...
    def Few(self, begin=1):
        ...
        for nit in range(begin, begin + 1):
            ...
            if self.dither is not None:
                self.scale = (self.random_number_generator.rand() * (self.dither[1] - self.dither[0]) +
                              self.dither[0])
            # 2
            for candidate in range(self.itersize):
                trials = np.array([self._mutate(c) for c in range(1)])
                ...
                if energy < self.population_energies[candidate]:
                    ...
                    if energy < self.population_energies[0]:
                        ...
                        if self.disp:
                            print("differential_evolution step %d-%d: f(x)= %g"
                                  % (nit, candidate, self.population_energies[0]))
                        self.success = self.callback(self._scale_parameters(self.population[0]),
                                                convergence=self.tol / convergence) is True
                        if (self.callback and self.success):
                            status_message = ('callback function requested stop early by returning True')
                            break
            DE_result = OptimizeResult(x=self.x, fun=self.population_energies[0],
                                       nfev=self._nfev,
                                       nit=nit + (candidate + 1) / self.itersize,
                                       success=self.success)
            if self.polish:
                result = minimize(self.func, np.copy(DE_result.x), method='L-BFGS-B',
                                  bounds=self.limits.T, args=self.args)
                self._nfev += result.nfev
                DE_result.nfev = self._nfev
    ...
    def Jump(self, begin=2):
        #3
        # do the optimisation.
        self.mutation_func = getattr(self, self._binomial['rand1bin'])
        ...
        self.mutation_func = getattr(self, self._binomial['best1bin'])
    ...
    def Fast(self, begin=2):
        ...
        for nit in range(begin, begin + 1):
            ...
            if self.disp:
                print("differential_evolution step %d: min = %g, max = %g"
                      % (nit,
                         self.population_energies[0], max(self.population_energies)))
    ...
    def AdaptiveDynamic_solve(self):
        ...
        if self.args[2]<-100:
            ...
            for candidate in range(self.itersize):
                ...
                if energy < lowest_energy:
                    ...
                    # should the solver terminate?
                    convergence = self.convergence
                    self.success = self.callback(self._scale_parameters(self.population[minval]),
                                                 convergence=self.tol / convergence) is True
                    if (self.callback and self.success):
                        status_message = ('callback function requested stop early by returning True')
                        DE_result = OptimizeResult(
                            x=self._scale_parameters(self.population[minval]),
                            fun=self.population_energies[minval],
                            nfev=self._nfev,
                            nit=nit + (candidate + 1) / self.itersize,
                            message=status_message,
                            success=self.success)
        ...
        for iteration in range(1, self.maxiter + 1):
            threshold = min(self.args[2], self.population_energies.mean() - self.args[3]*self.population_energies.std())
            energy_path.append(self.population_energies[0])
            if self.population_energies[0] < threshold:
                print('Few mode:', iteration, self.population_energies[0], '|', threshold)
                result = self.Few(begin=iteration)
            elif len(energy_path)>10 and np.std(energy_path[-10:])==0 and self.population_energies[0] == threshold:
                print('Convergence:', iteration, self.population_energies[0], '|', threshold)
                break
            # elif len(energy_path)>100 and np.std(energy_path[-100:])==0:
            #     print('Jump mode:', iteration, self.population_energies[0], '|', threshold)
            #     result = self.Jump(begin=iteration)
            else:
                print('Fast mode:', iteration, self.population_energies[0], '|', threshold)
    ...
    def AdaptiveDynamic_solve_1(self):
        nit, self.success = 0, False
        status_message = _status_message['success']
        if self.args[2]<-100:
            print('Fast mode:', 0)
        ...
        for iteration in range(1, self.maxiter + 1):
            ...
            attack_resultx = result.x
            pixels = len(attack_resultx) // 3
    ...
    def __next__(self):
        ...
        for trial in trials: self._ensure_constraint(trial)
        if self.args[1] != -1:
            for trial in trials: self.DEC(self.img, trial, self.args)
